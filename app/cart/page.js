...
export default function CartPage() { 
    ...
    if(cart){
        ...
        finalCart = cart[0].products.map((cartproduct) => {
              ...
              let prod =  products.find((product) => product.id == cartproduct.productId) ;
              prod["quantity"] =cartproduct.quantity
              
              return {id: prod.id, image:prod.image, title: prod.title, quantity: prod.quantity, price: prod.price}
              })
    ...
    return (
      <main className="flex min-h-screen flex-col items-center justify-between p-20 bg-white dark:bg-black">
        ...
        {
            isCartLoading && tempData.map((product) => 
            <Cart key={product}   isLoading={isCartLoading}/>
          )}
        {finalCart && finalCart.map((product) => 
            <Cart key={product.id} product={product}  isLoading={isCartLoading}/>
          )}
        </div>
        </section>
      </main>
